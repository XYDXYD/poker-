...
int money_decision()
{
	...
	double weight1 = (double)leastallmoney/ (leastallmoney + (playerList[meIndex].money + playerList[meIndex].jetton));
	double weight2 = (double)(playerList[meIndex].money + playerList[meIndex].jetton)/ (leastallmoney + (playerList[meIndex].money + playerList[meIndex].jetton));
		
	//结合
	sum_rate = weight1 * raise_rate + weight2 * my_rate;
	printf("<<<<<<<<<<<<<<<<<<<raise_rate%lf\n",raise_rate);
	printf("<<<<<<<<<<<<<<<<<<<my_rate%lf\n",my_rate);
	printf("<<<<<<<<<<<<<<<<<<<sum_rate%lf\n",sum_rate);
	//判断
	return (int)(100 * (1 - sum_rate ));
