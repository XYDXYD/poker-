...
DealMsg:
		...
		//	playerDecision = other_Players();  //1-100
			...
			playerDecision = 50;//other_Players();
			...
			else
			{
				...
				{	
					...
					{
						//比较好的情况 保证一定不会弃牌
						if(mycardDecision > 80 && playerDecision > 80)
						{
							snprintf(msg_to_server, sizeof(msg_to_server)-1, "call \n");
						}
						//比较坏的情况，保证一定弃牌
						else if( mycardDecision < 30 || playerDecision < 30 )
						{
							snprintf(msg_to_server, sizeof(msg_to_server)-1, "fold \n");
						}
						//中间情况，按照概率弃牌
				...
				else if(curr_orbiculus == FLOP)
				{
					...
					//如果两者都很好，选择加注 
					if(mycardDecision > 90 && playerDecision > 90)
					{
					...
					{
						//特殊情况先处理
						if( mycardDecision > 95 || playerDecision > 95 )
						{
							snprintf(msg_to_server, sizeof(msg_to_server)-1, "raise %d \n", lastBetMoneyDecision);
						}
						//非特殊情况					
						...
						{
							if(mycardDecision > 80)
							{
								finalDecision = 0.7 * mycardDecision + 0.3 * playerDecision;
							}
							else if(mycardDecision > 70)
							{
							...
							if(finalDecision > 70)
							{
								...
								{
									//需要跟的钱比我们剩余的钱多且我们的钱很多，防止把很多钱一次输光
									if( leastCallMoney > 0.5 * playerList[meIndex].jetton && playerList[meIndex].jetton > 750 || callLastBetMoneyDecision < 0)
										snprintf(msg_to_server, sizeof(msg_to_server)-1, "fold \n");
									else//包含正常call和强制all_in
										snprintf(msg_to_server, sizeof(msg_to_server)-1, "call \n");
								}
							}
							else if(finalDecision < 50)
							{
								snprintf(msg_to_server, sizeof(msg_to_server)-1, "fold \n");	
							}
							else 
